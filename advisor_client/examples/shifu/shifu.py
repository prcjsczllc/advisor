...
def main():
    ...
    shifuPath = parentPath + "/advisor_client/examples/shifu/autoMLTest"

    # Change model config
    with open("./ModelConfig.json","r+") as modelConfigFile:
        modelConfig = json.loads(modelConfigFile.read())
        for arg, value in vars(args).items():
            modelConfig["train"]["params"][arg] = [value]

    with open("./ModelConfig.json", "wt") as t:
        json.dump(modelConfig,t,indent=2,sort_keys=True)

    # Compute or learning
    subprocess.check_call(["cd " + "./autoMLTest" + "&& bash shifu train && > train.log"],shell=True)

    # Output the metrics
    logFileDir =  + "./train.log"
