...
def evalMetrics(modelConfig, shifuJobPath, package, metricInfo):
    ...
    if package == "shifu":
        ...
        if modelConfig["basic"]["runMode"] != "LOCAL":
            ...
            subprocess.call(["hadoop fs -get {1}/{2}/evals/{0}/EvalPerformance.json evals/{0}".format(
                modelConfig["evals"][0]["name"], modelConfig["basic"]["customPaths"]["hdfsModelSetPath"],
                modelConfig["basic"]["name"])], shell=True)
